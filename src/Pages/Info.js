...
class CardInfo extends CreateElement {
  constructor() {
    super();
    this.hookID = document.getElementById("section-info");
  }
  ...
  render(obj) {
    ...
    info.innerHTML = `
      ...
      <div class="section-info__overview-title">
        ${obj.title} <span class="rel-year">(${
      obj.release_date.split("-")[0]
    })</span>
      </div>
      <!--Overview About-->
      <div class="section-info__overview-about">
      ${obj.release_date} ${obj.genres.map((genre) => genre.name).join("/")} ${
      obj.runtime
    }min
      </div>
    ...
    const lang = this.renderElement("div", "section-info__facts-lang");
    const budget = this.renderElement("div", "section-info__facts-budget");
    const revenue = this.renderElement("div", "section-info__facts-revenue");

    status.textContent = `Status: ${obj.status}`;
    lang.textContent = `Language: ${obj.spoken_languages[0].name}`;
    budget.textContent = `Budget: ${formatter.format(obj.budget)}`;
    revenue.textContent = `Revenue: ${formatter.format(obj.revenue)}`;

    [status, lang, budget, revenue].forEach((item) => facts.appendChild(item));

    //Cast***
    let id = sessionStorage.getItem("cardId");
    const getData = new FetchData();
    getData.movieItems("movie", "credits", id).then((cast) =>
    ...
    getData.movieItems("movie", "images", id).then((img) => {
    ...
    getData.movieItems("movie", "videos", id).then((res) => {
    ...
    getData.movieItems("movie", "recommendations", id).then((res) => {
      res.slice(0, 4).forEach((recm) => {
        ...
        <div class="section-info__recomendations-box-title">
          <span class="rec-title">${recm.title}</span>
        </div>
      </div>
        `;
      });
    ...
    //Modal **
    getData.movieItems("movie", "videos", id).then((video) => {
